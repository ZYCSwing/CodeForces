...
bool check(int x, int y) {
    int r = 1;
    for (int i = x + 1; i < 10 && grid[i][y] == 'X'; ++i) {
        ++r;
    }
    ...
    r = 1;
    for (int i = y + 1; i < 10 && grid[x][i] == 'X'; ++i) {
        ++r;
    }
    ...
    r = 1;
    for (int i = x + 1, j = y + 1; i < 10 && j < 10 && grid[i][j] == 'X'; ++i, ++j) {
        ++r;
    }
    for (int i = x - 1, j = y - 1; i >= 0 && j >= 0 && grid[i][j] == 'X'; --i, --j) {
        ++r;
    }
    if (r >= 5) return true;

    r = 1;
    for (int i = x + 1, j = y - 1; i < 10 && j >= 0 && grid[i][j] == 'X'; ++i, --j) {
        ++r;
    }
    for (int i = x - 1, j = y + 1; i >= 0 && j < 10 && grid[i][j] == 'X'; --i, ++j) {
        ++r;
    }
    if (r >= 5) return true;
