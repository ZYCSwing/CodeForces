...
int main() {
    ...
    for (ll x = 1; x <= sqrt(n); ++x) {
        if (n % x) continue;
        ll tmp = n / x;
        maxres = max(maxres, cal(x, tmp, 1));
        maxres = max(maxres, cal(tmp, x, 1));
        for (ll y = sqrt(tmp); y >= 1; --y) {
            if (tmp % y == 0) {
                minres = min(minres, cal(x, y, tmp / y));
                break;
            }
        }
        for (ll y = sqrt(x); y >= 1; --y) {
            if (x % y == 0) {
                minres = min(minres, cal(tmp, y, x / y));
                break;
            }
        }
    }
