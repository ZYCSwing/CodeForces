...
int main() {
    char tmp[2];
    int row[2], col[2];
    scanf("%c%d", &tmp[0], &col[0]);
    getchar();
    scanf("%c%d", &tmp[1], &col[1]);
    row[0] = tmp[0] - 'a';
    row[1] = tmp[1] - 'a';
    col[0]--, col[1]--;
    ...
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            ...
            if ((abs(i - row[0]) == 1 && abs(j - col[0]) == 2) || (abs(i - row[0]) == 2 && abs(j - col[0]) == 1)) {
                --res;
                continue;
            }
            if (i == row[1] && j == col[1]) {
            ...
            if ((abs(i - row[1]) == 1 && abs(j - col[1]) == 2) || (abs(i - row[1]) == 2 && abs(j - col[1]) == 1)) {
                --res;
            }
        }
    }
