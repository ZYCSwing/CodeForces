...
void solve(int index) {
    int level0 = a[index], level1 = INT_MIN, level2;
    ss.erase(ss.find(a[index]));
    for (int i : edges[index]) {
        level1 = max(level1, a[i]);
        ss.erase(ss.find(a[i]));
    }
    ++level1;
    level2 = ss.empty() ? INT_MIN : *ss.rbegin() + 2;
    ...
    res = min(res, max(level0, max(level1, level2)));
}
